...
public class Pagamento : Entidade
{
    ...
    /// </summary>
    /// <returns>
    /// Uma string contendo o valor do pagamento, a forma de pagamento e, se aplicável, o número de parcelas.
    /// </returns>
    public override string ToString()
    {
        var text = new StringBuilder();
        text.Append($"Valor: {Valor.ToString(CultureInfo.CurrentCulture)}, ");
        text.Append($"Forma: {TipoPagamento.GetDisplayName()}, ");
        if (Parcelado)
            text.Append($"Parcelas: {QuantidadeParcelas}");
