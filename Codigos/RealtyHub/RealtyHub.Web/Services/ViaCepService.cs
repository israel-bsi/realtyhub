...
public class ViaCepService : IViaCepService
{
    ...
    /// <summary>
    /// Retorna um objeto <see cref="Endereco"/> com base no CEP informado, realizando a busca através do serviço ViaCep.
    /// </summary>
    /// <param name="cep">CEP a ser pesquisado.</param>
    /// <returns>
    /// Task contendo um <see cref="Response{Address}"/> com as informações de endereço ou mensagem de erro.
    /// </returns>
    public async Task<Response<Endereco?>> GetAddressAsync(string cep)
    {
        try
        {
            var searchAddressAsync = await SearchAddressAsync(cep);
            if (searchAddressAsync is { IsSuccess: false, Data: null })
                return new Response<Endereco?>(null, 500, searchAddressAsync.Message);

            var address = new Endereco
            {
                Cep = cep,
                Logradouro = searchAddressAsync.Data!.Street,
                Bairro = searchAddressAsync.Data.Neighborhood,
                Cidade = searchAddressAsync.Data.City,
                Estado = searchAddressAsync.Data.State,
                Complemento = searchAddressAsync.Data.Complement
            };

            return new Response<Endereco?>(address);
        }
        catch (Exception e)
        {
            return new Response<Endereco?>(null, 500, e.Message);
        }
    }
}